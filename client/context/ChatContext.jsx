...
export const ChatProvider = ({ children }) => {
  ...
  const createRoom = async (roomName, roomImage, category, description) => {
    try {
      console.log("room image", roomImage, "room name", roomName, "description", description, "category", category);
      const { data } = await axios.post('/api/messages/create-room', { roomName, roomImage, category, description });
      if (data.success) {
        console.log("room created successfully", data.createRoom);
      }
    }
    catch (error) {
      toast.error(error.message);
  ...
  const getPrivateRoom = async () => {
    try {
      const { data } = await axios.get('/api/messages/private-room');
      if (data.success) {
  ...
  const privateRoomMembers = async () => {
    try {
      ...
      if (data.success) {
        console.log("private room members", data.members);
        setPrivateMembers(data.members);
      }
    }
    catch (error) {
      toast.error(error.message);
