...
import { GroupMusicContext } from "../../../../context/GroupMusicContext";
...
const GlobalChatWindow = () => {
  ...
  const { selectedUser, messages, globalSendMessage, sendMessage, getRoomMessages, currentRoom ,song, 
    getSongs, } = useContext(ChatContext);
  ...
  // Check room and initialize socket rooms
  useEffect(() => {
    if (!currentRoom) {
      navigate("/global-room-lists");
    } else {
      joinVideoRoom(roomName); 
      joinMusicRoom(roomName); // Ensure we join the music socket room
    }
