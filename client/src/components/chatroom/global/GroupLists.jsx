...
const GroupsLists = () => {
  ...
  const { globalRoomLists, globalRoom, globalRoomJoin, joinedRooms, joinRoom, globalRoomMembers, getPrivateRoom, privateRoom, privateMembers, privateRoomMembers } = useContext(ChatContext);
    ...
    <div className="min-h-screen bg-[#f6f7fb] flex">
      ...
      <div className="hidden md:flex flex-col w-64 bg-white border-r p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button
            onClick={handleBack}
            className="flex cursor-pointer items-center justify-center min-w-[32px] h-8 w-8 rounded-full bg-gray-100 hover:bg-gray-200 active:scale-95 transition shadow-sm"
          >
        ...
        {/* Create Room (Desktop) */}
        {activeTab === "private" && (
          <button onClick={() => navigate('/create-room')} className="mt-auto bg-gradient-to-r from-pink-500 to-purple-500 text-white py-2 rounded-xl cursor-pointer">
            + Create Room
          </button>
        )}
      </div>
      ...
      <div className="flex-1">
        ...
        <div className="bg-white border-b px-4 py-3 flex flex-col gap-3 sticky top-0 z-20 md:px-6">
          ...
          <div className="flex justify-center">
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${activeTab === "public"
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all cursor-pointer ${activeTab === "private"
                  ? "bg-white text-pink-600 shadow-sm"
                  : "text-gray-500"
                  }`}
              >
          ...
          {/* Create Room (Mobile) */}
          {activeTab === "private" && (
            <button onClick={() => navigate('/create-room')} className="md:hidden bg-gradient-to-r from-pink-500 to-purple-500 text-white py-2 rounded-xl text-sm font-semibold">
              + Create Room
            </button>
          )}
        </div>
          ...
          {activeTab === "public" ? (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {isJoind(room.roomName) ? (
                          <button
                            ...
                            className="px-3 py-1.5 rounded-full bg-green-500 text-white text-xs font-semibold cursor-pointer"
                          >
                            Message
                          </button>
                        ) : (
            ...
            <>
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-500 text-sm">
                  Your private conversations and invite-only groups.
                </p>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {privateRoom.map((room, index) => (
                  ...
                  >
                    {/* Banner */}
                    <div className="h-28 md:h-32 w-full bg-gradient-to-r from-purple-500 to-indigo-500 overflow-hidden">
                      {room.roomImage ? (
                        <img
                          src={room.roomImage}
                          alt={room.roomName}
                    ...
                    <div className="p-3 md:p-4">
                      ...
                      <p className="text-sm text-gray-500 mb-4">
                        Private group for {room.roomName}.
                      </p>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {isPrivateRoomJoined(room.roomName) ? (
                          <button
                            onClick={() => {
                              joinRoom(room.id);
                              setTimeout(() => {
                                navigate(`/private-room/${room.roomName}`);
                              }, 500);
                            }}
                            className="px-3 py-1.5 rounded-full bg-green-500 text-white text-xs font-semibold cursor-pointer"
                          >
                          ...
                          <button
                            onClick={() => joiningRoom(room.roomName, room.id)}
                            className="px-3 py-1.5 rounded-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white text-xs font-semibold cursor-pointer"
                          >
                            Join
                          </button>
                        )}
      ...
      {activeTab === "private" && (
        <button onClick={() => navigate('/create-room')} className="fixed bottom-6 right-6 md:hidden bg-gradient-to-r from-pink-500 to-purple-500 text-white px-5 py-3 rounded-full shadow-lg">
          + Room
        </button>
      )}
