...
const ChatContainer = () => {
  ...
  return  selectedUser ?(
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        {messages.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !==authUser._id && 'flex-row-reverse'}`}>
            {msg.image ?(
              ...
              <p className={`p-2 max-w-[70%] break-words md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${msg.senderId==authUser._id ? 'rounded-br-none': 'rounded-bl-none'}`}>{msg.text}</p>
            )
            }
      ...
      <div className=' fixed bottom-0 left-0 right-0 flex items-center gap-3 p-3 '>
